...
| [`native-escape-hatch`](native-escape-hatch/) | `native()` with restraint: everything uses the portable API, and exactly one small file drops to `native()` for a server-side aggregate count the contract deliberately excludes, gated on the declared `aggregations` capability. A live test asserts the native count equals the portable one. |
